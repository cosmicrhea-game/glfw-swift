...
public final class GLFWWindow: GLFWObject {
  ...
  public var keyInputHandler:
    ((GLFWWindow, Keyboard.Key, Int, ButtonState, Keyboard.Modifier) -> Void)?
  ...
  public var cursorPositionHandler: ((GLFWWindow, Double, Double) -> Void)?
  public var mouseButtonHandler:
    ((GLFWWindow, Mouse.Button, ButtonState, Keyboard.Modifier) -> Void)?
  ...
  public func makeFullscreen(monitor: GLFWMonitor, size: Size, refreshRate: Int? = nil) {
    glfwSetWindowMonitor(
      pointer, monitor.pointer, .zero, .zero, Int32(size.width), Int32(size.height),
      refreshRate?.int32 ?? .dontCare)
  }
  ...
  public var margins: Margins {
    var left = Int32.zero
    var right = Int32.zero
    var top = Int32.zero
    var bottom = Int32.zero
  ...
  public func setSizeLimits(
    minWidth: Double?, minHeight: Double?, maxWidth: Double?, maxHeight: Double?
  ) {
    setSizeLimits(
      minWidth: minWidth.map(Int.init), minHeight: minHeight.map(Int.init),
      maxWidth: maxWidth.map(Int.init), maxHeight: maxHeight.map(Int.init))
  }
  ...
  public var contentScale: ContentScale {
    var xscale = Float.zero
    var yscale = Float.zero
  ...
  init(_ pointer: OpaquePointer!) {
    ...
    glfwSetKeyCallback(pointer) {
      let window = GLFWWindow.fromOpaque($0)
      window.keyInputHandler?(
        window, Keyboard.Key($1), $2.int, ButtonState($3), Keyboard.Modifier(rawValue: $4))
    }
    ...
    glfwSetCursorPosCallback(pointer) {
      let window = GLFWWindow.fromOpaque($0)
      window.cursorPositionHandler?(window, $1, $2)
    }
    ...
    glfwSetMouseButtonCallback(pointer) {
      ...
      window.mouseButtonHandler?(
        window, Mouse.Button(rawValue: $1) ?? .left, ButtonState($2),
        Keyboard.Modifier(rawValue: $3))
    }
  ...
  public convenience init(
    width: Int, height: Int, title: String, monitor: GLFWMonitor? = nil,
    sharedContext context: GLFWContext? = nil
  ) throws {
    let pointer = glfwCreateWindow(
      width.int32, height.int32, title, monitor?.pointer, context?.pointer)
